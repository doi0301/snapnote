...
- **자동 업데이트:** 배포 URL·`latest.yml` 호스팅이 없으면 불필요한 요청을 피하도록, 플레이스홀더(`example.com`)일 때는 **피드를 쓰지 않습니다** (`src/main/autoUpdate.ts`). 설정 화면에서 “업데이트 확인”을 누른 경우에만 검사합니다.
...
- **설정 화면 → 앱 정보·업데이트**에서 “업데이트 확인”으로 `latest.yml` 을 조회합니다. 새 버전이 있으면 `받기` 후 `지금 설치하고 재시작`으로 NSIS 설치를 실행합니다.
- `electron-builder.yml` 의 `publish.url` 이 실제 서버로 바뀌어 빌드되어야 합니다. 아직 `example.com` 이거나 메타가 없으면 피드가 설정되지 않은 것으로 안내합니다.
- 환경 변수:
  - `SNAPNOTE_DISABLE_AUTO_UPDATE=1` — 완전 끔
  - `SNAPNOTE_UPDATE_BASE_URL` — 피드 URL 강제 (스테이징 등)
  - `SNAPNOTE_ENABLE_AUTO_UPDATE=1` — 메타가 애매해도 피드 사용

서버에는 `electron-builder` 가 `dist/` 에 만든 `latest.yml` 과 설치 파일·`blockmap` 을 같은 베이스 URL 에 올려야 합니다 (GitHub Releases 등).
...
1. `electron-updater` 의존성 (이미 있음)
2. 메인 프로세스에서 `autoUpdater` 피드 설정 (`src/main/autoUpdate.ts`)
3. 빌드 산출물과 `latest.yml` 등을 동일 호스트에 배포
4. 사용자는 **설정**에서 업데이트 확인·다운로드·설치
