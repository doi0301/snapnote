...
- 중괄호는 화면에서 숨긴다 (`text` 에는 `{첨부}` 그대로 유지 — 저장/복사 포맷 불변,
  미러 레이어에서 `{`/`}` 를 시각적으로만 감춤)
...
**같이 고친다**: 새 규칙과 동일하게 `indentLevel <= titleIndent` 인 줄을 만나면
재정렬하지 말고 `break` 한다. 다만 이 조건에서는 마이그레이션이 사실상 무력화되므로,
**"legacy `sectionScope` 필드가 존재하는 문서에서만" 마이그레이션을 수행**하도록
게이팅하고, 그 외에는 건너뛴다.
